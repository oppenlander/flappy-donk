...
class Menu extends BaseState
  ...
  create: ->
    ...
    if not @game.babyMode
      ...
      descLabelName = 'A realistic barrel jumping simulator'
      modeLabelName = 'Baby Mode: B'
    else
      flappyLabel = @game.add.text(0, 50, 'FLAPPY DONK', {font: '84px VT323', fill: '#fff'})
      donkLabel = @game.add.text(0, 132, 'BABY MODE', {font: '84px VT323', fill: '#fff'})
      descLabelName = 'A realistic barrel jumping simulator for babies'
      modeLabelName = 'Normal Mode: B'

    flappyLabel.x = @gw/2 - flappyLabel._width/2
    donkLabel.x = @gw/2 - donkLabel._width/2
    descLabel = @game.add.text(0, 224, descLabelName, {font: '16px VT323', fill: '#fff'})
    ...
    modeLabel = @game.add.text(10, @gh-102, modeLabelName, {font: '24px VT323', fill: '#fff'})
  ...
  createDonkSelection: ->
    ...
    textDonkey = @game.add.text(0, 0, @selectDonkey.label, {font: '16px VT323', fill: '#fff'})
    ...
    textSquier = @game.add.text(0, 0, @selectSquier.label, {font: '16px VT323', fill: '#fff'})
    ...
    textKing = @game.add.text(0, 0, @selectKing.label, {font: '16px VT323', fill: '#fff'})
    ...
    textWoodfighter = @game.add.text(0, 0, @selectWoodfighter.label, {font: '16px VT323', fill: '#fff'})
